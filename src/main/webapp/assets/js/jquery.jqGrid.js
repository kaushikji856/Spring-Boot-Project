...
$.extend($.jgrid,{
	...
	parseDate : function(format, date, newformat, opts) {
		...
		opts.parseRe = /[#%\\\/:_;.,\t\s-]/;   //force parse regex
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		if(this.id!='listCorporateGrid') {
			$("#gbox_"+this.id).css("height","165px");
			var currentId = null;
			if(this.id=="lastRechargeGrid" || this.id=="lastActivationGrid" || this.id=="lastCommissionGrid") {
				currentId = 'lastPartnerTransaction';
			} else if(this.id=="vasTransHistGrid" || this.id=="vasUnsubDetailGrid") {
				currentId = 'accountVasGrid';
			} else if(this.id=="rechargeDetailsCDRsBinGrid" || this.id=="voiceDetailsCDRsBinGrid" || this.id=="smsDetailsCDRsBinGrid" || this.id=="vasDetailsCDRsBinGrid" || this.id=="allbrowsDetailsCDRsBinGrid" || this.id=="mmsDetailsCDRsBinGrid" || this.id=="balanceTransferCDRsBinGrid") {
				currentId = "accountRechargeGrid";
			} else {
				currentId = this.id;
			}
		
			$("#load_"+this.id).remove();
			$("#lui_"+this.id).remove();
		
			if(currentId=="searchMsisdnGrid" || currentId=="searchCorporateGrid" || currentId=="uploadedFilesGrid") {
				/*$("#loading_"+currentId).append("<div class='loadingGridDiv' style='width:100%; height:100%;' id='lui_"+this.id+"'> <div style='border : none; margin-top: 85px;' id='load_"+this.id+"'><img height='65px' src='../../images/loading-animation.gif' /><input type='hidden' id='call_"+this.id+"' value='1' /></div> </div>");*/
				$("#loading_"+currentId).append("<div class='loadingGridDiv' style='width:100%; height:100%;' id='lui_"+this.id+"'> <div style='border : none; margin-top: 85px;' id='load_"+this.id+"'><div class='loading ui-state-default ui-state-active' style='float:left; padding:8px 20px; position: relative; left:45%;'><span>Loading...</span></div><input type='hidden' id='call_"+this.id+"' value='1' /></div> </div>");
			} else {
				/*$("#loading_"+currentId).append("<div class='loadingGridDiv' style='width:100%; height:100%;' id='lui_"+this.id+"'> <div style='border : none; margin-top: 85px;' id='load_"+this.id+"'><img height='65px' src='../images/loading-animation.gif' /><input type='hidden' id='call_"+this.id+"' value='1' /></div> </div>");*/
				$("#loading_"+currentId).append("<div class='loadingGridDiv' style='width:100%; height:100%;' id='lui_"+this.id+"'> <div style='border : none; margin-top: 85px;' id='load_"+this.id+"'><div class='loading ui-state-default ui-state-active' style='float:left; padding:8px 20px; position: relative; left:45%;'><span>Loading...</span></div><input type='hidden' id='call_"+this.id+"' value='1' /></div> </div>");
			}
			
			if(this.id=="createActivityGrid" || this.id=="createSrGrid") {
				$('#load_createActivityGrid').css("margin-top","60px");
				$('#load_createSrGrid').css("margin-top","60px");
			}
			
			/*$("<div class='ui-widget-overlay jqgrid-overlay' id='lui_"+this.id+"'></div>").insertBefore(gv);*/
			
			/*$("<div class='ui-widget-overlay jqgrid-overlay' style='background-color: white !important;' id='lui_"+this.id+"'></div>").insertBefore(gv); my change*/
			
			$("<div class='ui-widget-overlay jqgrid-overlay'></div>").insertBefore(gv);
			
			/*$("<div class='loading ui-state-default ui-state-active' id='load_"+this.id+"'>"+this.p.loadtext+"</div>").insertBefore(gv);*/
			$("<div class='loading ui-state-default ui-state-active'></div>").insertBefore(gv);
			
			/*$("<div class='loading ui-state-default ui-state-active' style='border : none; top : 30%;' id='load_"+this.id+"'><img height='65px' src='../images/loading-animation.gif' /></div>").insertBefore(gv); my change*/
		}
		...
		endReq = function() {
			...
			switch(ts.p.loadui) {
				...
				case "enable":
					...
					$("#lui_"+$.jgrid.jqID(ts.p.id)).remove();
					$("#load_"+$.jgrid.jqID(ts.p.id)).remove();
		...
		var columnSet = null;
		if(this.id=='listCorporateGrid') {
			columnSet = "<table class='ui-jqgrid-htable' style='line-height: 16px; width:"+ts.p.tblwidth+"px' role='grid' aria-labelledby='gbox_"+this.id+"' cellspacing='0' cellpadding='0' border='0'></table>";
		} else {
			columnSet = "<table class='ui-jqgrid-htable' style='width:"+ts.p.tblwidth+"px' role='grid' aria-labelledby='gbox_"+this.id+"' cellspacing='0' cellpadding='0' border='0'></table>";
		}
		//var hTable = $("<table class='ui-jqgrid-htable' style='width:"+ts.p.tblwidth+"px' role='grid' aria-labelledby='gbox_"+this.id+"' cellspacing='0' cellpadding='0' border='0'></table>").append(thead),
		var hTable = $(columnSet).append(thead),
...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		/*TODO TnM Starts by Karthik changed pop up after activity creation from Error to Status*/
		/*$(mh).append("<span class='ui-jqdialog-title'>"+p.caption+"</span>");*/
		$(mh).append("<span class='ui-jqdialog-title'>"+"Status"+"</span>");
		...
		if (editable==="1") {
			...
			$('td[role="gridcell"]',ind).each(function(i) {
				...
				if ( nm !== 'cb' && nm !== 'subgrid' && cm.editable===true && nm !== 'rn' && !$(this).hasClass('not-editable-cell')) {
					...
					if(cv[0] === false) {
						//commented for validation in sr
						//cv[1] = tmp[nm] + " " + cv[1];
						return false;
					}
			...
			if(o.add) {
				$($t).jqGrid('navButtonAdd', elem,{
					...
					onClickButton : function () {
						var accountStatus = $('.activeMark').attr('title');
						if((accountStatus=='Initial' || accountStatus=='Sold') && (gID=='createSrGrid' || gID=='createSrGridSumm')) {
							alert("You can not create SR for "+ accountStatus +" Number");
							return false;
						} else {
							...
							if(!o.addParams.useFormatter) {
								...
								$("#"+gID+"_ilNoRecord").css("display","none");
							}
						}
			...
			if(o.cancel) {
				$($t).jqGrid('navButtonAdd', elem,{
					...
					onClickButton : function () {
						...
						$("#"+$t.p.id+"_ilNoRecord").css("display","table-row");
						if(sr) {
							if($("#"+$.jgrid.jqID(sr), "#"+gID ).hasClass("jqgrid-new-row")) {
